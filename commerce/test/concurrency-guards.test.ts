...
class LosingItemMutationDb extends LosingBatchDb {
  constructor(
    private readonly items: any[],
    private readonly originalItem: any | null = null,
  ) {
    super();
  }
  ...
  prepare(query: string): Statement {
    ...
    if (query.includes("FROM order_revision_items")) {
      return new Statement(query, null, this.items);
    }

    if (query.includes("FROM order_items") && query.includes("WHERE id = ?")) {
      return new Statement(query, this.originalItem);
    }
...
function confirmedItem(overrides: Record<string, unknown> = {}) {
  return {
    id: 1,
    lineNumber: 1,
    sourceOrderItemId: 11,
    catalogProductId: "HC-004",
    sku: "LP75454",
    slug: "hc-004-highland-cow-in-bath-ornament",
    productName: "Highland Cow Soaking",
    unitPriceMinor: 950,
    requestedQuantity: 1,
    confirmedQuantity: 1,
    availabilityStatus: "CONFIRMED",
...
describe("Admin V2 concurrency guards", () => {
  ...
  it("rejects a same-version draft update loser and token-guards item/event side effects", async () => {
    const db = new LosingItemMutationDb([confirmedItem()]);
  ...
  it("rejects a concurrent add-item loser and token-guards inserted item/event", async () => {
    const db = new LosingItemMutationDb([
      confirmedItem({
        catalogProductId: "HC-004",
      }),
    ]);
  ...
  it("rejects a concurrent remove-added-item loser and token-guards delete/event", async () => {
    const db = new LosingItemMutationDb([
      confirmedItem({
        id: 1,
        lineNumber: 1,
  ...
  it("rejects a concurrent restore-original loser and token-guards restore/event", async () => {
    const originalItem = {
      id: 11,
      lineNumber: 1,
      catalogProductId: "HC-004",
