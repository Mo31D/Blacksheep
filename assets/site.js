...
function itemUrl(item,type){return item?.commerceDynamic===true?'/product.html?type='+encodeURIComponent(type||item.type||'gifts')+'&slug='+encodeURIComponent(item.slug):'/products/'+encodeURIComponent(item.slug)+'.html'}
function productImageSrc(item,src=item?.img){if(!src)return'';const value=String(src);if(/^https?:\/\//i.test(value))return value;if(value.startsWith('/'))return value;return '/images/'+value}
