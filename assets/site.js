...
function polishListButtons(root=document){root.querySelectorAll('.list-add').forEach(btn=>{if(btn.dataset.listButtonReady==='1')return;btn.dataset.listButtonReady='1';if(/^\s*\+?\s*My list\s*$/i.test(btn.textContent||''))btn.textContent='Add to list'})}
